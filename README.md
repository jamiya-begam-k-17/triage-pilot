...
- Optional: a [Groq](https://console.groq.com) API key, only needed if
  ...
  fallback (see [LLM Triage Notes](#llm-triage-notes) below).
...
Screenshots below are from a real run of `streamlit run streamlit_app.py`
against the sample queue in `challenge/referral-queue.json` — nothing
here is mocked or staged.

**1. Landing page** — no run has happened yet, nothing is faked on load.

![Landing page](docs/screenshots/01_landing.png)

**2. Dashboard** — after clicking **▶ Run workflow**: counts of
`COMPLETED` / `ESCALATED` / `HANDOFF` / `FAILED`, plus the referral
queue underneath.

![Dashboard](docs/screenshots/02_dashboard.png)

**3. Referral queue** — searchable, filterable, one row per referral,
with a **View →** button per row.

![Referral queue](docs/screenshots/03_referral_queue.png)

**4. Referral detail (HANDOFF)** — clicking **View →** jumps straight
to that referral's detail page. This one shows a hard-stop banner and
the caseworker handoff package for a household with a minor.

![Referral detail — handoff](docs/screenshots/04_referral_detail.png)

**5. Referral detail (COMPLETED)** — the generated triage note, with
an explicit `Deterministic fallback` / `provider` badge so it's always
clear whether an LLM wrote it.

![Referral detail — completed](docs/screenshots/06_completed_note.png)

**6. Referral detail (ESCALATED)** — the supervisor escalation
package, clearly stating the action was blocked and what a human must
do next.

![Referral detail — escalated](docs/screenshots/07_escalated_package.png)

**7. Execution trace** — the full UNDERSTAND → HISTORY → DECIDE → ACT
audit trail for the run, filterable by referral.

![Execution trace](docs/screenshots/05_execution_trace.png)

### What an evaluator should verify

- Clicking **▶ Run workflow** actually executes `app.workflow.run_workflow()`
  (not a mock) — the dashboard counts should match
  `pytest`/`python -m app.main` output for the same queue.
- The `HANDOFF` referral above (`RF-2026-0412`) has **no triage note**
  anywhere — not in the UI, not in `artifacts/triage-notes/`.
- The `ESCALATED` referral above (`RF-2026-0413`) has **`Action taken:
  NONE`** in its escalation package — the restricted action was never
  partially performed.
- The `COMPLETED` referral's triage note badge matches what's written
  in `artifacts/triage-notes/RF-2026-0417.md` (`Generated by:` /
  `Provider:` header).
- Re-running the workflow is idempotent and doesn't require restarting
  the app.
...
To generate notes with an LLM instead, TriagePilot can use Groq:
...
- **"Connection refused" / history lookups fail** — if running the backend directly,
  make sure the mock history service is running on port 8083. The Streamlit UI
